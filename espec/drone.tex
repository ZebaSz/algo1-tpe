...
\asegura{(ds == \lvacia \wedge \res == \lvacia) \vee (mismos(res, [ cruce \ | \ i \leftarrow [0..\longitud{vueloRealizado(ds_0)}), \\ cruce \leftarrow [ (posicion, \longitud{[ 1 | d \leftarrow ds, vueloRealizado(d)_i == posicion]}) | \\ posicion \leftarrow posicionesSinRepetir([ vueloRealizado(d)_i | d \leftarrow ds ]) ], \sgd{cruce} > 1 ]))}
