...
\asegura[mataPlagas]{estadoDelCultivo(\pre{s}, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == ConPlaga \\ \implica (usaPlaguicidaComun(d, \pre{d}) \wedge bateria(d) == bateria(\pre{d}) - 11 \\ \wedge estadoDelCultivo(s, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == RecienSembrado) \\ \vee (usaPlaguicidaBajoConsumo(d, \pre{d}) \wedge bateria(d) == bateria(\pre{d}) - 6 \\ \wedge estadoDelCultivo(s, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == RecienSembrado) \\ \vee (\neg tienePlaguicida(d) \wedge noHaceNada(d, \pre{d}) \\ \wedge estadoDelCultivo(s, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == ConPlaga))}
