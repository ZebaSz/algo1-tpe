...
%(estadoDelCultivo(s, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == RecienSembrado \wedge bateria(d) == bateria(\pre{d}) - 6 )}


%si ((hay maleza) y (tiene algun herbicida)) entonces ((usa herbicida o usa herbicidalargoalc) y siembra y gasta bateria correspondiente)
%si ((hay plaga) y (tiene algun plaguicida)) entonces ((usa plaguicida y gasta bateria o usa plagicidabajocons y gasta poca bateria) y siembra)

\asegura[desplaguizado]{estadoDelCultivo(\pre{s}, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == ConPlaga \implica(bateria(\pre{d}) > 11 \implica (usaPlaguicidaComun(d, \pre{d}) \wedge ) ) (estadoDelCultivo(s, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == RecienSembrado \wedge bateria(d) == bateria(\pre{d}) - 6 )}
