...
\asegura[fertilizarCampo]{(\forall i \in [0..\prm{dimensiones(campo(s))})) (\forall j \in [0..\sgd{dimensiones(campo(s))})) \\ contenido(campo(s), i, j) == Cultivo \implica \\ (((estadoDelCultivo(\pre{s}, i, j) == RecienSembrado \vee estadoDelCultivo(\pre{s}, i, j) == EnCrecimiento) \implica estadoDelCultivo(s, i, j) == ListoParaCosechar) \\ \wedge (estadoDelCultivo(\pre{s}, i, j) \neq RecienSembrado \wedge estadoDelCultivo(\pre{s}, i, j) \neq EnCrecimiento) \implica estadoDelCultivo(s, i, j) == estadoDelCultivo(\pre{s}, i, j)))}

\asegura[mismosDrones]{\longitud{enjambreDrones(s)} == \longitud{enjambreDrones(\pre{s})}}

\asegura[cambiosEnDrones]{(\forall d \in enjambreDrones(\pre{s})) \\ (enVuelo(d) \wedge ((\exists e \in enjambreDrones(s)) id(e) == id(d)  \wedge enVuelo(e) \wedge alOeste(e, d) \\ \wedge bateria(e) == (bateria(d) + \longitud{vueloDiferente(e,d)}) \\ \wedge unFertilizantePorCultivo(e,d))) \wedge soloCultivos(s,vueloDiferente(e,d))  \\ \vee (\neg enVuelo(d) \wedge \en{d,enjambreDrones(s))}}
