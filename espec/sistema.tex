...
\asegura {campo (\res) == c}
\asegura[mismosDrones]{\longitud{ds} == \longitud{enjambreDrones(\res)} \wedge \ (\forall \ d1 \in ds) \\ (\exists \ d2 \in enjambreDrones(\res)) \ mismoDrone(d1, d2)}
\end{problema}
...
\asegura[mismosOtrosDrones]{\longitud{enjambreDrones(\pre{s})} == \longitud{enjambreDrones(s)} \wedge \\ (\forall d1 \in enjambreDrones(\pre{s}), \ id(d1) \neq id(d)) \ (\exists d2 \in enjambreDrones(s)) \ mismoDrone (d1, d2)}
...
\asegura[vuelosAlOeste]{(\forall d \in enjambreDrones(\pre{s}), enVuelo(d)) (\exists e \in enjambreDrones(s), id(e) == id(d)) \\ vueloRealizado(e) == vueloRealizado(d) ++ [ \\ (\prm{vueloRealizado(d)_{\longitud{vueloRealizado(d)} - 1}} - (i - \longitud{vueloRealizado(d)}), \\ \sgd{vueloRealizado(d)_{\longitud{vueloRealizado(d)} - 1}}) \ | \ i \rightarrow [\longitud{vueloRealizado(d)}..\longitud{vueloRealizado(e)})] \\ \wedge \ posicionActual(e) == vueloRealizado(e)_{\longitud{vueloRealizado(e)} - 1}}
...
\asegura[condicionDeFinalizacion]{((\forall d \in enjambreDrones(s), enVuelo(d)) \\ bateria(d) == 0 \vee \neg(\en{Fertilizante, productosDisponibles(d)}) \\ \vee \prm{vueloRealizado (d)_{|vueloRealizado(d)| - 1}} == 0) \\ \vee contenido(campo(s), \prm{vueloRealizado (d)_{|vueloRealizado(d)| - 1}} - 1, \\ \sgd{vueloRealizado (d)_{|vueloRealizado(d)| - 1}}) \neq Cultivo}
