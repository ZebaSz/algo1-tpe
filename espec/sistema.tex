...
\asegura[desplaguizado]{estadoDelCultivo(\pre{s}, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == ConPlaga \implica(bateria(\pre{d}) > 11 \implica) (estadoDelCultivo(s, \prm{posicionActual(d)}, \sgd{posicionActual(d)}) == RecienSembrado \wedge bateria(d) == bateria(\pre{d}) - 6 )}
